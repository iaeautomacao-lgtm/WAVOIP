...
@app.route("/api/debug-smtp-actual")
def debug_smtp_actual():
    steps = []
    import time
    try:
        import smtplib
        ...
        nome = "Caio"
        instituicao = "UNIVERSIDADE VEIGA DE ALMEIDA"
        forma_pagamento = "Boleto"
        valor = ""
        link_boleto = ""
        link_pix = ""
        linha_dig = "34191.09065 85620.120387 72039.040000 4 14960000259893"
        vencimento = "03/07/2026"
        
        secao_pix = ""
        boleto_html = '<span style="color:#667085;">Link de boleto indisponivel.</span>'
        secao_boleto = f"""
        <tr>
...
<tr><td style="padding:32px 36px;">
  ...
  <p style="color:#475467;font-size:13px;margin:24px 0 0;line-height:1.6;">
    ...
    <strong>Equipe de Atendimento – DDM</strong>
        ...
        msg["Subject"] = "[DEBUG ACTUAL] Acordo formalizado"
        msg["From"] = smtp_from
        ...
        t0 = time.time()
        try:
            steps.append("Resolving DNS...")
            host_ipv4 = socket.gethostbyname(smtp_host)
            steps.append(f"DNS resolved in {time.time()-t0:.2f}s: {host_ipv4}")
        except Exception as e:
            steps.append(f"DNS resolve failed: {e}")
            
        t0 = time.time()
        steps.append("Connecting to SMTP...")
        if smtp_security == "ssl":
            server = smtplib.SMTP_SSL(host_ipv4, smtp_port, timeout=15)
        else:
            server = smtplib.SMTP(host_ipv4, smtp_port, timeout=15)
        steps.append(f"Connected in {time.time()-t0:.2f}s")
        ...
        with server:
            ...
            if smtp_security == "starttls":
                ...
                server.starttls()
                server.ehlo()
            steps.append(f"EHLO/TLS done in {time.time()-t0:.2f}s")
            
            t0 = time.time()
            steps.append("Logging in...")
            server.login(smtp_user, smtp_pass)
            steps.append(f"Logged in in {time.time()-t0:.2f}s")
            
            t0 = time.time()
            steps.append("Sending mail...")
            server.sendmail(smtp_from, [debug_recipient], msg.as_string())
            steps.append(f"Mail sent in {time.time()-t0:.2f}s")
